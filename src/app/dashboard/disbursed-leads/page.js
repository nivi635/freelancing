  ...
  return (
    <div className="p-4">
      <div className="flex gap-2 mb-4">
        <button
          className={`border px-4 py-2 rounded ${view === "filter" ? "bg-gray-300" : ""}`}
          onClick={() => setView("filter")}
        >
          <i className="bx bx-filter-alt"></i> Filter
        </button>
        <button
          className={`border px-4 py-2 rounded ${view === "chart" ? "bg-gray-300" : ""}`}
      ...
      {view === "filter" ? (
        <div className="bg-white rounded-lg statuscard-shadow p-6 mt-3 flex flex-col md:flex-row lg:flex-row gap-4 lg:gap-10">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 p-3 w-full">
            <Dropdown placeholder={"All Banks"} label={"Banks"} options={options} />
            <Dropdown placeholder={"Select Tier"} label={"Tier"} options={options} />
            <Dropdown placeholder={"All Products"} label={"Products"} options={options} />
            <Dropdown placeholder={"All Team Members"} label={"AssignedTo"} options={options} />
            <Dropdown placeholder={"Month-Year"} label={"Months"} options={getLatestMonths()} />
            <div className="border border-gray-400 flex justify-center w-full bg-white min-w-[150px]">
              <label className="text-center text-gray-600 border border-gray-400 p-2">Day</label>
          ...
          <div className="flex justify-center items-end p-3 gap-4 w-full md:w-auto lg:w-auto">
            <button className="border border-gray-400 px-4 py-2 rounded-sm text-gray-600">Clean</button>
      ...
      <div className="mt-4 text-gray-600 p-5 border-b-2 border-gray-400">No Leads Available.</div>
    </div>
  );
}
...
function Dropdown({ placeholder, label, options }) {
  return (
    <div className="border border-gray-400 flex justify-center w-full bg-white min-w-[150px]">
      <label className="text-center border text-gray-600 border-gray-400 p-2">{label}</label>
      <div className="relative w-full">
        <select className="appearance-none border px-2 py-2 rounded w-full pr-8 bg-white text-sm" defaultValue="">
